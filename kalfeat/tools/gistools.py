...
def _resume_count(deg_or_min, value):
...
def convert_position_str2float(position_str):
    ...
    if len(p_list) != 3:
        raise ValueError(
            f'{position_str} not correct format,should be DD:MM:SS')

    deg = float(p_list[0]) 
    minutes = float(p_list[1])
    sec = float(p_list[2])
    
    minutes, sec = _resume_count(minutes, sec) 
    deg, minutes =  _resume_count(deg, minutes) 
...
#             " kalfeat.utils.gistools.project_point_ll2utm() should be "
#             "used instead.")
...
@gdal_data_check
@deprecated("This function may be removed in later release."
            " kalfeat.utils.gistools.project_point_utm2ll() should be "
            "used instead.")
